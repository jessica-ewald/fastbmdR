...
#' @return A `fitObj` object containing the input data and curve fitting results.
#' @export
...
#' @param num.sds A `numeric` specifying the number of standard deviations to define the BMR.
...
PerformBMDCalc <- function(fitObj, ncpus = 1, num.sds = 1, bmr.method = "sample.mean", log10.dose = FALSE)
{
  ...
  if (!inherits(fitObj, "fitObjFilt"))
    stop("Use only with 'fitObjFilt' objects, created with the function FilterDRFit")
...
#' @param log10.dose A `logical` specifying whether the dose is on a log scale.
#' @param num.sds A `numeric` specifying the number of standard deviations to define the BMR.
#' @param filt.var A `character` that specifies the variable used for selecting the best curve fit.
#' @return A `data.frame` with curve fit and BMD results.
#' @export
